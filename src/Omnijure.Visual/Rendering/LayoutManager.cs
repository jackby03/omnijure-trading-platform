...
public class LayoutManager
{
        ...
        Omnijure.Core.DataStructures.OrderBook orderBook)
    {
        ...
        _sidebar.Render(canvas, SidebarRect.Width, SidebarRect.Height, orderBook);
        canvas.Restore();
        
        // 3. Draw Chart
        canvas.Save();
        canvas.ClipRect(ChartRect);
        renderer.Render(canvas, (int)ChartRect.Width, (int)ChartRect.Height, buffer, decision, scrollOffset, zoom, symbol, interval, chartType, buttons, minPrice, maxPrice, mousePos);
        canvas.Restore();
    }
}
